...
- **ReDoS is bounded, not prevented.** Untrusted project patterns are capped in count and length, but neither Node's nor Bun's `RegExp` has a timeout. A catastrophic-backtracking pattern from a repository can still stall a decision. Use `trustedProjects` only for repositories you trust. The global file is not capped at all, so the same care applies to patterns you write by hand. Three shapes have been measured stalling this gate, all of them found in its own published sample: `\S+=\S*`, which can split one shell word at any `=` where `[^\s=]+=\S*` cannot; two greedy `[^\n]*` scans in one pattern, whose cost grows cubically; and one greedy `[^\n]*` scan, which restarts at every occurrence of the term before it and so grows quadratically. A rule that means “this command mentions A and mentions B” is linear when written as two anchored lookaheads and nothing else — `^(?=[\s\S]*A)(?=[\s\S]*B)` — because each lookahead runs once and is atomic. `test/samples.test.ts` times every rule in the published sample against 64 KB of input built from that rule's own words for exactly this reason.
